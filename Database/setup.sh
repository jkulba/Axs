...
SQL_SCRIPT="/tmp/create.sql" # Assuming you'll copy it into the container

# Copy the SQL script into the container and then execute
echo "Copying SQL script to the container..."
podman cp create.sql "$CONTAINER_NAME:$SQL_SCRIPT"
...
if [ $EXIT_CODE -eq 0 ]; then
  echo "Database 'Accessdb', table 'AccessRequests' created successfully inside the container."
else
